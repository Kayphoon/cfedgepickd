...
Options:
  --version VERSION   Release tag to install, for example v0.2.16. Default: latest
  --emergency-rtt-ms MS
...
install_from_dir() {
  package_dir="$1"
  if [ ! -x "$package_dir/tunnelflux" ]; then
    echo "tunnelflux binary not found in $package_dir" >&2
    exit 1
  fi

  if [ "$mode" = "dry-run" ]; then
    "$package_dir/tunnelflux" install --protocol "$protocol" --emergency-rtt-ms "$emergency_rtt_ms" --config "$config" --binary "$prefix/tunnelflux" --unit "$unit"
    exit 0
  fi
  ...
  install -m 0755 "$package_dir/tunnelflux" "$prefix/tunnelflux"

  "$prefix/tunnelflux" install --apply --protocol "$protocol" --emergency-rtt-ms "$emergency_rtt_ms" --config "$config" --binary "$prefix/tunnelflux" --unit "$unit"
  ...
  echo "installed TunnelFlux; inspect with: tunnelflux status"
  if [ "$start_service" != "true" ]; then
...
if [ -x "$script_dir/tunnelflux" ]; then

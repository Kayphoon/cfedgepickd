...
TunnelFlux 是一个面向 `cloudflared` Tunnel 边缘 IP 选择的小型控制面守护进程和命令行工具。
...
当配置为 `protocol: auto` 时，TunnelFlux 可以用 QUIC 探测给 IP 排名，但会继续让生产 `cloudflared` 保持 `auto`，这样 UDP/QUIC 后续异常时仍可回退到 HTTP/2。
...
安装器会自动识别 Linux/macOS 和 `amd64`/`arm64`，下载匹配的发布包，在可用时校验 `checksums.txt`，安装二进制文件，写入 `/etc/tunnelflux/config.json`，写入平台对应的服务定义，启用守护进程并启动它。

使用 `--dry-run` 可以只预览发现结果和计划写入内容，不改动机器。使用 `--no-start` 可以只安装而不启动服务。安装时传 `--lang zh` 或 `--zh` 会让 `tf status` 默认使用中文标签；不传或传 `--lang en` 则默认英文。
...
一行安装脚本是正常安装路径。它会写入 `/etc/tunnelflux/config.json`，安装 systemd unit 或 launchd plist，启用守护进程并启动它。
...
守护进程每轮都会向 `/var/lib/tunnelflux/history.jsonl` 追加一条 JSONL 记录。默认每 5 分钟采样一次，由 `switching.probe_interval_seconds` 控制。历史文件路径可通过 `runtime.history_file` 配置。

历史记录默认保留 30 天，由 `runtime.history_retention_days` 控制。每次成功追加后，旧记录会被清理。把这个值设为负数可以关闭清理。

使用 `tf status` 可以查看终端仪表盘。它会展示统一状态总览、当前边缘连接、边缘 IP 对比、最近决策状态和按时间排序的折线图。默认图表会同时显示请求率和错误率。默认语言来自安装器写入配置中的 `runtime.language`，也可以在单次命令中用 `--lang` 或 `--zh` 覆盖：
...
每个压缩包都包含 `tf`、`install.sh`、systemd service 文件和示例配置文件。`tf` 是唯一的运行和运维入口。
...
标签构建会把标签名嵌入 `tf version`。非标签 CI 构建会嵌入分支或 ref 名称以及短提交 SHA。
...
手动执行 `tf switch --apply` 会有意绕过 degraded、cooldown 和 idle 门槛，因为这是明确的人工操作。它默认使用 blue/green 热切换；需要旧的重启式路径时可使用 `--mode restart`。

所有 hosts/config 写入都会先备份。如果非活跃槽位在超时前没有达到 `readyConnections >= 2`，TunnelFlux 会停止它，恢复备份，并保持旧的活跃槽位继续运行。
